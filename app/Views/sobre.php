...
<!-- Seção Sobre a Empresa - Ajustada para telas pequenas -->
<section class="max-w-4xl mx-auto px-4 py-8 md:py-12">
    <div class="text-center mb-8 md:mb-12">
        <h1 class="text-2xl md:text-4xl font-extrabold text-gray-900 tracking-tight">Sobre o FreelaApp</h1>
        <p class="text-sm md:text-md text-gray-500 mt-2">Conectando contratantes aos melhores profissionais liberais do mercado.</p>
    </div>

    <div class="bg-white p-5 md:p-8 rounded-2xl shadow-sm border border-gray-100 space-y-4 md:space-y-6 text-sm md:text-base text-gray-600 leading-relaxed">
...
<section class="max-w-xl mx-auto px-4 py-8 md:py-12 border-t border-gray-100">
    <div class="bg-white p-5 md:p-8 rounded-2xl shadow-sm border border-gray-100">
        <h2 class="text-xl md:text-2xl font-bold text-gray-900 text-center">Fale Conosco</h2>
        <p class="text-xs md:text-sm text-gray-500 text-center mt-1">Dúvidas, críticas ou sugestões? Envie uma mensagem.</p>

        <?php if (isset($_GET['contato']) && $_GET['contato'] === 'sucesso'): ?>
            <div class="bg-emerald-50 text-emerald-700 p-4 rounded-xl text-xs md:text-sm font-medium mt-4 border border-emerald-100 text-center">
                ✉️ Mensagem enviada com sucesso! Responderemos em breve.
            </div>
        <?php endif; ?>

        <form action="/contato/enviar" method="POST" class="mt-6 space-y-4">
            <div>
                <label class="block text-xs md:text-sm font-medium text-gray-700">Seu Nome</label>
            ...
            <div>
                <label class="block text-xs md:text-sm font-medium text-gray-700">E-mail para Retorno</label>
            ...
            <div>
                <label class="block text-xs md:text-sm font-medium text-gray-700">Mensagem</label>
